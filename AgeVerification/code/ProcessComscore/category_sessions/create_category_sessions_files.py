...
#          XVIDEOS.COM from the full set of desktop merged session files.

"""
Loops through every merged_sessions_{YYYYMM}.parquet file one at a time,
...
Outputs:
  ...
  data/ProcessComscore/merged_cat_session_files/xnxx_sessions.parquet
  data/ProcessComscore/merged_cat_session_files/xvideos_sessions.parquet

Usage:
  python code/ProcessComscore/create_category_sessions_files.py
...
SITES = {
    "PORNHUB.COM": "pornhub_sessions",
    "XNXX.COM":    "xnxx_sessions",
    "XVIDEOS.COM": "xvideos_sessions",
}
...
session_files = sorted(
    glob.glob(os.path.join(sessions_dir, "merged_sessions_??????.parquet"))
)
print(f"\nFound {len(session_files)} monthly session files")
...
for fpath in session_files:
    yyyymm = os.path.basename(fpath).replace("merged_sessions_", "").replace(".parquet", "")
