"""
Session duration CDFs and concentration curves:
  1. Replicate key plots from full data (all years, all sessions)
...
WINDOW_DAYS = 90   # days before/after law passage
...
for slug, display_name in SITES:
    print(f"\n{'─'*55}\n{display_name}")
    df = pd.read_parquet(
        f"data/ProcessComscore/merged_cat_session_files/{slug}_sessions.parquet"
    )
    dur_all = df["duration"].dropna()
    p95_all = np.percentile(dur_all, 95)
    N_all   = len(dur_all)

    note_base = f"{display_name} desktop, 2022–2024 (N={N_all:,.0f})."
    ...
    fig.suptitle(f"CDF of {display_name} session duration — Desktop, 2022–2024",
    ...
    fig.savefig(f"{OUTDIR}/full_duration_cdf_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved full_duration_cdf_{slug}.png")
    ...
    fig2.text(0.5, -0.03,
              ...
              ha='center', fontsize=8, color='#555555')
    fig2.savefig(f"{OUTDIR}/full_duration_concentration_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig2)
    print(f"  Saved full_duration_concentration_{slug}.png")
    ...
    for (label, series), color in zip(pages_groups(df, dur_all).items(), COLORS6):
        x, y = cdf_xy(series, CAP_PG)
        ax.plot(x, y, color=color, lw=1.8, label=f"{label}  (n={len(series):,})")
    fmt_min(ax); spine_clean(ax)
    ax.set_xlabel("Session duration (minutes)", fontsize=10)
    ax.set_ylabel("Cumulative share of sessions", fontsize=10)
    ...
    fig3.text(0.5, -0.03,
              f"X-axis truncated at 8 min. {note_base}",
              ha='center', fontsize=8, color='#555555')
    fig3.savefig(f"{OUTDIR}/full_duration_cdf_by_pages_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig3)
    print(f"  Saved full_duration_cdf_by_pages_{slug}.png")

    # ── 4. Pre vs. post — treated states ±90 days around law passage ──────────
    # Law date = first post=1 date per state
    ...
    print(f"  Pre window:  N={len(dur_pre):,}   Post window: N={len(dur_post):,}   Control: N={len(dur_ctrl):,}")

    # Use a common p95 (from pre window) for concentration curves
    p95_pre = np.percentile(dur_pre, 95)
    ...
    fig4.suptitle(
        f"CDF of {display_name} session duration — Treated states pre vs. post law",
        fontsize=12, fontweight='bold', y=1.02)
    fig4.text(0.5, -0.04,
              f"±{WINDOW_DAYS}-day window around each state's law date. "
              f"Control states shown over full 2022–2024 period.",
              ha='center', fontsize=8, color='#555555')
    fig4.savefig(f"{OUTDIR}/prepost_duration_cdf_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig4)
    print(f"  Saved prepost_duration_cdf_{slug}.png")
    ...
    # custom legend: solid=minutes, dotted=sessions
    ...
    fig5.text(0.5, -0.03,
              f"±{WINDOW_DAYS}-day window around each state's law date. "
              f"Winsorized at pre-law P95.",
              ha='center', fontsize=8, color='#555555')
    fig5.savefig(f"{OUTDIR}/prepost_concentration_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig5)
    print(f"  Saved prepost_concentration_{slug}.png")
