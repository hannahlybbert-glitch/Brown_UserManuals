"""
Session duration plots for Pornhub, Xvideos, and XNXX (desktop, Jan-Mar 2022).
...
def cdf_xy(series, cap, denom=None):
    """Sorted values and cumulative shares up to cap. denom defaults to len(series)."""
    if denom is None:
        denom = len(series)
...
for slug, display_name in SITES:
    df  = pd.read_parquet(f"data/ProcessComscore/merged_cat_session_files/{slug}_sessions.parquet")
    ...
    print(f"\n{'─'*50}")
    print(f"{display_name}: N={N:,}  median={dur.median():.0f}s  P95={p95:.0f}s  >30min={pct_over_30m:.1f}%")

    note_base = f"{display_name} desktop sessions, Jan–Mar 2022 (N={N:,.0f})."
    ...
    fig.suptitle(f"CDF of {display_name} session duration — Desktop, Jan–Mar 2022",
                 fontsize=12, fontweight='bold', y=1.02)
    fig.text(0.5, -0.04,
             note_base + f" {pct_over_30m:.1f}% of sessions exceed 30 min (excluded from right panel).",
             ha='center', fontsize=8, color='#555555')
    fig.savefig(f"{OUTDIR}/duration_cdf_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved duration_cdf_{slug}.png")
    ...
    fig2.text(0.5, -0.03,
              ...
              ha='center', fontsize=8, color='#555555')
    fig2.savefig(f"{OUTDIR}/duration_concentration_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig2)
    print(f"  Saved duration_concentration_{slug}.png")
    ...
    fig3.text(0.5, -0.03,
              ...
              ha='center', fontsize=8, color='#555555')
    fig3.savefig(f"{OUTDIR}/duration_histogram_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig3)
    print(f"  Saved duration_histogram_{slug}.png")
    ...
    for (label, mask), color in zip(pages_groups.items(), COLORS6):
        ...
        x, y = cdf_xy(group_dur, CAP_4, denom=len(group_dur))
        ax.plot(x, y, color=color, lw=1.8, label=f"{label}  (n={len(group_dur):,})")
    ...
    fig4.text(0.5, -0.03,
              ...
              ha='center', fontsize=8, color='#555555')
    fig4.savefig(f"{OUTDIR}/duration_cdf_by_pages_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig4)
    print(f"  Saved duration_cdf_by_pages_{slug}.png")
