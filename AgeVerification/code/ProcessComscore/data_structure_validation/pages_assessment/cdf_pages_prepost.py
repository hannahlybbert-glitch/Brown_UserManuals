"""
Pre vs. post law plots for treated states (±90-day window):
...
for slug, display_name in SITES:
    ...
    # Three pages groups to keep it readable; solid = pre, dashed = post
    CAP = 480  # 8 min
    pages_groups = {
    ...
    ax.set_title(f"CDF of session duration by pages — {display_name}\nTreated states: pre (solid) vs. post (dashed) law",
    ...
    fig.savefig(f"{OUTDIR}/prepost_cdf_by_pages_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved prepost_cdf_by_pages_{slug}.png")
    ...
    ax.set_title(f"Distribution of pages per session — {display_name}\nTreated states: pre vs. post law",
                 fontsize=11, fontweight='bold')
    spine_clean(ax)
    ...
    fig2.text(0.5, -0.03,
              ...
              f"±{WINDOW}-day window.",
              ha='center', fontsize=8, color='#555555')
    fig2.savefig(f"{OUTDIR}/prepost_pages_dist_{slug}.png", dpi=150, bbox_inches='tight')
    plt.close(fig2)
    print(f"  Saved prepost_pages_dist_{slug}.png")

    # ── C. TE on sessions and minutes by session-length bucket ────────────────
    # Buckets: [0,1), [1,2), ..., [14,15), [15+] minutes
    edges_min = list(range(0, 16)) + [np.inf]   # in minutes
    edges_sec = [e * 60 for e in edges_min]      # convert to seconds
    ...
    def bucket_stats(subset):
        ...
        for i in range(len(labels_b)):
            ...
            mask = (dur >= lo) & (dur < hi)
            counts[i]  = mask.sum()
            minutes[i] = dur[mask].sum() / 60   # convert to minutes
    ...
    # Proportional effects (% change relative to pre baseline)
    ...
    def bar_color_arr(vals):
        return [PALETTE[0] if (v is not None and not np.isnan(v) and v >= 0) else PALETTE[1] for v in vals]
    ...
    for mode in ['absolute', 'proportional']:
        if mode == 'absolute':
            ...
            fname   = f"prepost_te_buckets_{slug}.png"
            ...
            cnt_lbl = "% change\n(sessions)"
            min_lbl = "% change\n(total minutes)"
            fname   = f"prepost_te_buckets_pct_{slug}.png"
        ...
        axes[0].set_title(
            ...
            f"{display_name} — Treated states, ±{WINDOW}-day window — {title_sfx}",
            fontsize=11, fontweight='bold')
        spine_clean(axes[0])
