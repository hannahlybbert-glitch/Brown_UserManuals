...
# Created: 02/23/2026
# Purpose: Aggregate one month of sessions to machine_id x week_of_sample, split by coarse category

"""
Machine-Level Monthly Aggregation (Script 4 of 5)
Reads the intermediate session file for one calendar month and aggregates to
...
Usage:   python 4_aggregate_machine_month.py YYYYMM
Example: python 4_aggregate_machine_month.py 202201

Input:
  data/Aggregation/intermediate/{month_id}/intermediate_sessions_YYYYMM.parquet

Output (9 files per month, 324 total):
  data/Aggregation/machine_panel/monthly/{coarse_category}/
      machine_month_YYYYMM_{coarse_category}.parquet
...
if len(args) != 1:
    print("Usage: python 4_aggregate_machine_month.py YYYYMM [--test]")
    print("Example: python 4_aggregate_machine_month.py 202201")
    sys.exit(1)

yyyymm = args[0]

print("=" * 80)
print(f"MACHINE-LEVEL MONTHLY AGGREGATION: {yyyymm}" + (" [TEST MODE]" if TEST_MODE else ""))
print("=" * 80)
...
# Find the intermediate file for this month (sits inside a month_id subdirectory)
matches = glob(
    os.path.join(intermediate_dir, "*", f"intermediate_sessions_{yyyymm}.parquet")
)
if len(matches) == 0:
...
print(
    f"  Week range: {sessions['week_of_sample'].min()} "
    f"to {sessions['week_of_sample'].max()}"
)
print(f"  Unique machines: {sessions['machine_id'].nunique():,}")

# Categories present in this month's data
...
for cat in all_coarse_categories:
    ...
    if len(cat_sessions) == 0:
        # No sessions for this category this month — save empty file so script 5
        # can safely glob without special-casing missing months
    ...
    # Aggregate to (machine_id, week_of_sample): sum duration
    agg = (
        cat_sessions
...
print(f"Output location: {output_base}/{{category}}/machine_month_{yyyymm}_{{category}}.parquet")
print("=" * 80)
print("COMPLETE")
print("=" * 80)
