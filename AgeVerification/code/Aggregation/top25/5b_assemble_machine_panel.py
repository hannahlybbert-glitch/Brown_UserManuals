...
# Created: 2026-05-21
# Purpose: Assemble final desktop machine-level panel for top-25 categories.
#          Mirrors 5_assemble_machine_panel.py.
#
# Reuses the existing machine_week_presence.parquet and full demographics from
# the original aggregation pipeline — no need to rebuild those.
#
# Usage: python3 code/Aggregation/top25/5b_assemble_machine_panel.py
#
# Input:
#   data/Aggregation/top25/machine_panel/monthly/{category}/
#       machine_month_{YYYYMM}_{category}.parquet            (from 4b, 936 files)
#   data/Aggregation/machine_panel/machine_week_presence.parquet          [existing]
#   data/ProcessComscore/full_demographics/full_machine_person_demos.parquet [existing]
...
print("=" * 80)
print("ASSEMBLE TOP-25 MACHINE PANEL (DESKTOP)")
print("=" * 80)

# ----- FILE PATHS ----- #
panel_dir    = os.path.join(project_root, "data", "Aggregation", "top25", "machine_panel")
...
# Reuse existing presence and demographics — not category-specific
...
# Build presence MultiIndex for vectorized lookup
...
for cat_idx, cat in enumerate(all_coarse_categories, 1):
    ...
    if len(monthly_files) == 0:
        ...
        print("         Please run 4b for all 36 months first.")
        continue
