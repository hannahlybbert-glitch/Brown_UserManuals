...
# Created: 2026-05-21
# Purpose: Create top-25 intermediate session files — XXX Adult sessions only,
#          categorised as one of the top 25 sites or other_adult.
#
# Mirrors 2_create_intermediate_sessions.py but tracks the top 25 adult sites
# (from top25_adult_sites.csv) instead of the original top 5.
# Non-XXX-Adult sessions are dropped entirely.
#
# Usage: python3 code/Aggregation/top25/2b_create_intermediate_sessions.py YYYYMM
#
# Input:
...
if len(sys.argv) != 2:
    print("Usage: python3 code/Aggregation/top25/2b_create_intermediate_sessions.py YYYYMM")
    sys.exit(1)

yyyymm = sys.argv[1]

print("=" * 80)
print(f"TOP-25 INTERMEDIATE SESSIONS (DESKTOP): {yyyymm}")
print("=" * 80)

# ----- FILE PATHS ----- #
sessions_file = os.path.join(
    project_root, "data", "ProcessComscore", "merged_session_files",
    f"merged_sessions_{yyyymm}.parquet"
)
...
output_dir = os.path.join(
    project_root, "data", "Aggregation", "top25", "intermediate", str(month_id)
)
