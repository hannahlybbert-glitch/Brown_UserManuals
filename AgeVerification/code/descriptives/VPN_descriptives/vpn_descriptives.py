...
# Created: 03/09/2026
# Purpose: Summary statistics and descriptives on VPN usage concentration
#          among post-treatment XXX visitors in treated states.

"""
VPN Concentration — Descriptive Statistics
==========================================

Prints a formatted summary to stdout and saves one histogram figure.

Stats produced:
  1. Sample overview (machine counts by group)
  2. VPN site prevalence — all machines, treated vs control
  3. VPN prevalence pre vs post law (treated states excl. TX)
  4. VPN prevalence by XXX visitor status
  5. Cross-tab: treated × XXX visitor × VPN type
  6. VPN visit frequency distribution (histogram + summary stats)

Groups:
  treated       — machines in qualifying treated states, excl. TX
  TX            — Texas (law immediately enjoined, reported separately)
  control       — machines in states with no age-verification law
  XXX visitor   — machine ever visited any of the 5 main XXX sites
                  or other_XXX_sites (total_duration > 0)

Pre/post law split (treated states excl. TX):
  Each machine's law week is determined by its home state's effective date.
  Pre  = any VPN visit in week_of_sample < state law week
  Post = any VPN visit in week_of_sample >= state law week

Run from project root:
  python3 code/analysis/vpn_descriptives.py

Outputs:
  stdout — formatted tables
  output/analysis/vpn_concentration/descriptives/vpn_frequency_histogram.png

# =============================================================================
# NOTE: the CSV-generating table code below is commented out but preserved.
# Uncomment to regenerate the per-state, per-site CSV tables.
# =============================================================================
"""
...
XXX_SITES_ADULT = [        # used to define "XXX visitor" (excludes all_other_sites)
    "PORNHUB.COM",
    "XVIDEOS.COM",
    "XHAMSTER.COM",
    "XNXX.COM",
...
out_dir  = os.path.join(project_root, "output", "descriptives", "VPN_descriptives",
                        "vpn_concentration", "descriptives")
...
# Per-machine law week (for treated machines only, used for pre/post split)
...
for vpn_type in VPN_TYPES:
    ...
    # Pre/post split for treated machines (excl TX)
    ...
    # VPN visit frequency: distinct weeks per machine (all machines that ever visited)
    ...
    # Session duration (seconds per machine-week), converted to minutes
...
W = 60
s = vpn_stats   # shorthand

print("\n")
print("=" * W)
print("  VPN DESCRIPTIVE STATISTICS — AgeVerification Sample")
print("=" * W)
...
CAP = 15   # cap display at 15 weeks; remainder shown as 15+
...
for ax, vpn_type in zip(axes, VPN_TYPES):
    ...
    bins = list(range(1, CAP + 2))  # 1 through CAP+1 (last bin = CAP+)
    ...
    ax.set_title(f"{vpn_type}: VPN visit frequency — Desktop")
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f"{y:.1%}"))
    ax.legend(fontsize=9)

plt.suptitle("How often do VPN site visitors actually visit VPN sites?",
             fontsize=13, y=1.01)
plt.tight_layout()
...
# Bin edges in minutes: 0-1, 1-2, ..., 9-10, 10-20, 20-30, ..., 50-60, 60+
DUR_CAP    = 60
...
for ax, vpn_type in zip(axes, VPN_TYPES):
    ...
    ax.set_title(f"{vpn_type}: VPN session duration — Desktop")
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f"{y:.1%}"))
    ax.legend(fontsize=9)

plt.suptitle("How long do machines spend on VPN sites per week?",
             fontsize=13, y=1.01)
plt.tight_layout()
...
# =============================================================================
# COMMENTED OUT: per-state, per-site CSV tables
# Uncomment and run to regenerate cohort_sizes.csv, vpn_penetration_{type}.csv,
# vpn_timing_{type}.csv, vpn_frequency_{type}.csv, state_summary_{type}.csv
# =============================================================================

# XXX_SITES_ALL = [
#     "PORNHUB.COM", "XVIDEOS.COM", "XHAMSTER.COM", "XNXX.COM",
#     "CHATURBATE.COM", "other_XXX_sites", "all_other_sites",
# ]
# T_MIN, T_MAX = -16, 8
#
...
#
# # --- TABLE 1: cohort sizes (VPN-independent) ---
# # t1_rows = []
# # for xxx_site in XXX_SITES_ALL:
# #     xxx_pos = (pd.read_parquet(...).query("total_duration > 0")[...])
# #     ...
# #     t1.to_csv(os.path.join(out_dir, "cohort_sizes.csv"), ...)
#
# # --- TABLES 2, 3, 4, 6: VPN-dependent ---
# # for vpn_type in VPN_TYPES:
# #     ...
# #     add_pooled_and_save(t2_rows, ..., f"vpn_penetration_{vpn_type}.csv")
# #     add_pooled_and_save(t3_rows, ..., f"vpn_timing_{vpn_type}.csv")
# #     add_pooled_and_save(t4_rows, ..., f"vpn_frequency_{vpn_type}.csv")
# #     t6.to_csv(os.path.join(out_dir, f"state_summary_{vpn_type}.csv"), ...)
